{{-- resources/views/admin/strategic-plan/edit.blade.php --}}
@extends('admin.layouts.admin-layout', ['activeMenu' => 'strategic-plan'])

@section('title', 'Edit Strategic Plan')
@section('page-title', 'Edit Strategic Plan')
...
<div class="form-card">
    <form action="{{ route('admin.strategic-plan.update', $strategicPlan) }}" 
          method="POST"
        ...
        <div class="form-group">
            ...
            <input type="text" 
                   name="title" 
                   class="form-input" 
                   value="{{ old('title', $strategicPlan->title) }}"
        ...
        <div class="form-group">
            ...
            <textarea name="description" 
                      ...
                      required>{{ old('description', $strategicPlan->description) }}</textarea>
            @error('description')
                <div class="error-message">{{ $message }}</div>
            @enderror
        ...
        <div class="form-group">
            ...
            <select name="category" class="form-select" required>
                ...
                <option value="tata_kelola" {{ old('category', $strategicPlan->category) === 'tata_kelola' ? 'selected' : '' }}>
                    Tata Kelola
                </option>
                <option value="program_layanan" {{ old('category', $strategicPlan->category) === 'program_layanan' ? 'selected' : '' }}>
                    Program & Layanan
                </option>
            </select>
            @error('category')
        ...
        <div class="form-group">
            ...
            <input type="number" 
                   name="order" 
                   class="form-input" 
                   value="{{ old('order', $strategicPlan->order) }}"
                   placeholder="Masukkan urutan tampilan"
                   min="1">
            @error('order')
                <div class="error-message">{{ $message }}</div>
            @enderror
            <div class="form-help">Urutan tampilan di halaman publik</div>
        </div>
        ...
        <div class="form-group">
            <div class="checkbox-wrapper">
                <input type="checkbox" 
                       ...
                       id="is_active"
                       value="1"
                       {{ old('is_active', $strategicPlan->is_active) ? 'checked' : '' }}>
